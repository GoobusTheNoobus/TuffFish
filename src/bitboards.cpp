...
Bitboard raycast_bishop(Square square, Bitboard blockers) {
    ...
    for (auto vec: bishop_vectors) {
        for (int i = 1; i <= 7; ++i) {
            int newr = r + vec.first * i;
            int newf = f + vec.second * i;

            if (out_of_bounds(newr, newf)) break;

            Square squ = parse_square(Rank(newr), File(newf));

            mask |= SQUARE_BB[squ];

            if (blockers & SQUARE_BB[squ]) 
                break;
        }
    }
    return mask;
